    ...
    */
    ...
    function encodeHTML(str) {
        if (typeof str == 'string') {
            ...
            // Note: Line below was originally uncommended.
            // str = str.replace(/[^ -~]/g, '');
            return str;
        } else {
            return '';
            ...
            return function() {
                ...
                if (string === '') {
                    ...
                    if (position === 0) {
                        ...
                        cursor.html(encodeHTML(string[position]));
        ...
        var keyboard_event = (function() {
            ...
            return function(e) {
                if (enabled) {
                    ...
                    if (e.keyCode == 13) {
                    ...
                    } else if (e.keyCode == 9 && !e.metaKey) { /* TAB */
                        options.tabcomplete(command, position, 
                            options.terminal, function(extra) {
                            // console.log("extra:", extra);
                            if (extra) {
                                command = command.slice(0, position) + extra + command.slice(position);
                    ...
                    } else if (history && e.keyCode == 38 ||
                               (e.keyCode == 80 && e.ctrlKey)) {
                        //UP ARROW or CTRL+P
                        self.set(history.previous());
                    } else if (history && e.keyCode == 40 ||
                    ...
                    } else if (e.keyCode == 123) { //F12 - Allow Firebug
                        return true;
                    ...
                    } else if (e.keyCode == 76 && e.metaKey) { // CTRL+L clears the terminal
                        ...
                        if (e.shiftKey) { // CTRL+SHIFT+??
                            ...
                            if (e.keyCode == 65) {
                                ...
                                //CTRL+E
                                self.position(command.length);
                            } else if (e.keyCode == 88 || e.keyCode == 67 ||
                                       e.keyCode == 87 || e.keyCode == 84) {
                            ...
                            } else if (e.keyCode == 70) { //CTRL+F find
                                return true;
                            } else if (e.keyCode == 17 || e.keyCode == 9) { //CTRL+TAB switch tab
                                return true;
                            }
                        }
                    } else if (e.altKey) {
                        //if (e.keyCode == 18) { // press ALT
                        if (e.keyCode == 68) { //ALT+D
                            pos = position;
                            len = command.length;
                            var space = null;
                            for (var i=pos; i<len; ++i) {
    ...
    $.fn.terminal = function(init_eval, options) {
        ...
        (function() {
            ...
            function scroll_to_bottom(terminal) {
                terminal.attr({scrollTop: self.attr("scrollHeight")});
            }
            $.extend(self, {
                clear: function() {
                    output.html('');
        ...
        if (valid('prompt', settings.prompt) && 
            ...
            self.command_line = self.find('.terminal-output').next().cmd({
                ...
                commands: commands, 
				tabcomplete: settings.tabcomplete, 
				terminal: self
            });
